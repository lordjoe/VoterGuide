 ...
 *            http://www.gnu.org/copyleft/copyleft.html for details on copyleft)
 */
public class Matrix implements Cloneable // SLEWIS allow cloning
{
	/**
	 * Matrix is an array of double (rows,cols). I've had no success making this a template.
	 */

    // SLewis - this is a constant need not be an instance variable
    private static final double		TINY	= 0.00001;
    public static final Random RND = new Random();

     // SLewis - added a few ways to build matrices
    /**
     * make a matrix with randon data between lowerBound and
     *    upperBound
     * @param rows number rows
     * @param cols  number rows
     * @param lowerBound see above
     * @param upperBound see above
     * @return non-null matrix as above
     */
    public static Matrix randomMatrix(int rows,int cols,double lowerBound,double upperBound)
    {
     ...
     * @param size number rowa and cols
     * @return non-null matrix as above
     */
     ...
     * @param values - values for the diagonal
     * @return non-null matrix as above
     */
 ...
 // SLewis - we can get these from matrix
//	private int			rows;
//	private int			cols;
	private double[][]	matrix; //	= null SLewis - null is default
	...
	public Matrix( int rows ) {
        this(rows,1);    // SLewis - use existing constructor
//		setRows( rows );
//		setCols( 1 );
//		setMatrix( rows, 1 );
	}
...
//		}
//		return response;
          // SLewis - throwing an exception is the proper error handling methos
...
//	}
	...
	/*
	 * Make sure we're accessing legitimate components.
	 */
...
//	}
     ...
     * @param matrix2 non-null matrix to compare
     * @return  as above
     */
    public boolean sameSize(Matrix matrix2)
    {
	 ...
	 * within the tolerances supplied.
	 *
	 * @param obj
	 *            other matrix
	 * @param tiny
	 ...
	 */
  //  public boolean equals( Object obj, double tiny ) {
    // SLEWIS - do not overriee equals and hash code for matrices
...
//		return equals( obj, TINY );
//	}
//
//	public int hashCode() {
//		return this.getRows() + this.getRows() * this.getCols() + this.getCols();
//	}
