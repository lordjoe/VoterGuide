...
public abstract class ClassAnalyzer
{
    ...
    public static Object getProperty(Object TheTarget, String name)
    {
        ...
        String[] items = Util.parseString(name, ".");
    ...
    protected static Object getSingleProperty(Object TheTarget, Class TheClass, String name)
    {
        ...
        try {
            ...
            throw new WrapperException("Error getting property " + name, RealException);
        }
        return (ret);
    }
    ...
    public static void setProperties(Object TheTarget, Map Values)
    {
        ...
        String[] items = Util.collectionToStringArray(Values.keySet());
        try {
            for (int i = 0; i < items.length; i++) {
                PropName = items[i];
                                                   ...
                                                   Object... args)
    {
        ...
        try {
            ...
            //throw new WrapperException(ex.getTargetException());
            ...
            throw new WrapperException(Message, ex2);
        }
   ...
   ========================================================= */
                                                             ...
                                                             Method[] methods)
    {
        ...
        for (int i = 0; i < possibleCollections.size(); i++) {
            ...
            if (ICollectionHolder.class.isAssignableFrom(TheClass) && "CollectionMember".equals(
                    ColectionName))
                continue; // well known not to be
                                                 ...
                                                 Map<String, ClassProperty> caselessTb)
    {
        ...
        BackedByField bf = test.getAnnotation(BackedByField.class);
        if (bf != null)
            backingFieldName = bf.FieldName();
        for (int i = 0; i < fields.length; i++) {
            Field field = fields[i];
    ...
    protected static Object convertStringToDesiredType(Class dataType, String sValue)
    {
        ...
        if (dataType == String.class) {
            ...
            if (sValue.indexOf(":") > -1)
                value = TimeDuration.decodeElapsedTimeString(sValue);
            else
                value = new Long(sValue);
        }
        else if (EnumeratedType.class.isAssignableFrom(dataType)) {
            value = EnumeratedType.stringToValue(dataType, sValue);
        }
