...
public class DocumentReportElement implements INameable
{
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        Document item = XMLUtil.buildDOM(filename);
        DocumentReportElement report = new DocumentReportElement(item);
        for (int i = 1; i < args.length; i++)
        {
            String arg = args[i];
            item = XMLUtil.buildDOM(filename);
            report.addCorrespondingElement(item);
        }
        String xml = report.generateXML();
        System.out.println(xml);
        FileUtilities.writeFile("UsedFields.xml", xml);
    }
